...
export default function CommentBoard(props) {
    ...
    // make some changes here to use useCallback()
    // think about why we add three parameters to the function
    // there are 2 steps 
    // 1. define the function when the component is mounted
    // 2. call the function and pass the parameters when the button is clicked(at the same time, we will get the value that we need)
    const  handleLoginSubmit = useCallback((e, user, password) => {
        e?.preventDefault();
    ...
    }, []); // this function will only be created when the component is mounted

    // when the comments change, it will recalculate the length
    const len = useMemo(() => {
        return comments.length;
    }, [comments]); // this value will be updated when the comments change
